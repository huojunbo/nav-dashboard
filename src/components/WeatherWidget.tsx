...
type GeoStatus = 'pending' | 'success' | 'denied' | 'error';
...
const WeatherWidget = (): React.JSX.Element => {
  ...
  const [locationName, setLocationName] = useState<string>(t('weather.detectingLocation'));
  ...
  useEffect(() => {
    ...
    const fetchLocationName = async (): Promise<void> => {
      try {
  ...
  const getWeatherIcon = (code: number): React.JSX.Element => {
  ...
  const getSmallIcon = (code: number): React.JSX.Element => {
